...
def set_material(obj, color):
    assert type(color) == tuple and len(color) == 4, "Color must be a 4-tuple (R, G, B, A)"

    # Create a new material
    mat = bpy.data.materials.new(name="Diffuse Color")
    # Set the material's diffuse color
    mat.diffuse_color = color
    # Reduce specularity and glossiness
    mat.specular_intensity = 1  # Reduce specularity (range 0-1)
    mat.roughness = 0.5  # Increase roughness to reduce glossiness (range 0-1)
...
# Seems like this is not easily parallelizable because of the dependence on bpy for view angles
def generate_views():
...
if __name__ == "__main__":
    ...
    obj_filepath = './data/cube.obj'
    mtl_filepath = './data/cube.mtl' # This is just for reference; Blender will automatically search for the .MTL file in the same directory as the .OBJ
    save_dir = './data/red_cube_smooth'
    ...
    # resize ball to be 1/4 the size; zoom in camera, by zooming in the lens by 4x
    adjust_sizing(0.5)
