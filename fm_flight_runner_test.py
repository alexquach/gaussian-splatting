...
def closed_loop_render_set(gaussians, pipeline, background, init_conditions, inference_configs, record_hz,
                           closed_loop_save_path, params_path=None, checkpoint_path=None, object_colors=None,
                           keycamera_path=None, PYBULLET_TO_GS_SCALING_FACTOR=None, text_instr=None, selected_index=None):
    ...
    # ! Setup Simulator
    sim = EvalSimulator(render_path, init_conditions, record_hz, selected_index)
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, inference_configs: dict,
                scene_config: dict, init_conditions, text_instr, selected_index):
    ...
    with torch.no_grad():
        ...
        if is_closed_loop:
            for closed_loop_save_path, params_path, checkpoint_path, record_hz in tqdm(
                    zip(closed_loop_save_paths, params_paths, checkpoint_paths, inference_configs["record_hzs"]),
                    desc="Closed loop rendering progress"):
                closed_loop_render_set(gaussians, pipeline, background, init_conditions, inference_configs, record_hz,
                                       closed_loop_save_path, params_path, checkpoint_path, objects_color,
                                       keycamera_path, PYBULLET_TO_GS_SCALING_FACTOR, text_instr, selected_index)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), inference_configs, scene_config,
                init_conditions, args.text_instr, args.selected_index)
